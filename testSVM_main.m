function accuracy = testSVM_main(trainX,trainY,testX,testY,num_classes,kernel,options)
...
for i = 1:num_classes
    ...
    if strcmp(kernel,'rbf')
        SVM = fitcsvm(vecTrainX,tempTrainY,'KernelFunction',kernel,'KernelScale',options);
    elseif strcmp(kernel, 'polynomial')
        SVM = fitcsvm(vecTrainX,tempTrainY,'KernelFunction',kernel,'PolynomialOrder',options);
    else
        SVM = fitcsvm(vecTrainX,tempTrainY,'KernelFunction','linear');
    end
