function accuracy = testLR(X,Y,solver,k)
% This function will test the average accuracy of Logistic Regression using
...
for i = 1:k
    ...
    if strcmp(solver, 'fitclinear')
        LR = fitclinear(trainX,trainY,'Learner','logistic');
        fun = @(x) 1 / (1 + exp(-(x*LR.Beta + LR.Bias)));
    elseif strcmp(solver, 'cvx')
        [w,b] = myLR(trainX,trainY);
        fun = @(x) 1 / (1 + exp(-(x*w + b)));
    else
